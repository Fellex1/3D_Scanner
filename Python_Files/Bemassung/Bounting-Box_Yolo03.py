from ultralytics import YOLO
import cv2

model = YOLO("yolo12m.pt")  # mittlere Version

cap = cv2.VideoCapture(0)

ret, frame = cap.read()
cap.release()

if not ret:
    print("Fehler: Kein Bild erhalten.")
    exit()

results = model.predict(frame, imgsz=1280)[0]

for box in results.boxes:
    x1, y1, x2, y2 = map(int, box.xyxy[0])
    width = x2 - x1
    height = y2 - y1

    # Bounding Box zeichnen
    cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)

    # ---- Breite oben mittig ----
    width_text = f"{width}px"
    (w_text_width, w_text_height), _ = cv2.getTextSize(width_text, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
    cv2.putText(frame, width_text, (x1 + (width - w_text_width) // 2, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

    # ---- Höhe links mittig ----
    height_text = f"{height}px"
    (h_text_width, h_text_height), _ = cv2.getTextSize(height_text, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
    cv2.putText(frame, height_text, (x1 - h_text_width, y1 + (height + h_text_height) // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)




# Anzeigen
cv2.imshow("Boxes", frame)
cv2.waitKey(0)
cv2.destroyAllWindows()
